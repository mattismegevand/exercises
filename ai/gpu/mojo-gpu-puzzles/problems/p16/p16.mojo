...
fn matmul_tiled[
    ...
    if tiled_row < size and tiled_col < size:
        for k in range((size + TPB - 1) // TPB):
            a_shared[local_row, local_col] = a[tiled_row, k]
            b_shared[local_row, local_col] = b[k, tiled_col]
            barrier()

            output[tiled_row, tiled_col] += a_shared[local_row, local_col] * b_shared[local_row, local_col]
